...
const Dashboard = () => {
    ...
    return (
        <div className="min-h-screen pt-20 bg-background-soft flex pb-24 lg:pb-0">
            ...
            {/* Bottom Nav (Mobile) */}
            <div className="lg:hidden fixed bottom-4 left-4 right-4 h-20 bg-white/90 backdrop-blur-xl border border-white/20 shadow-2xl rounded-3xl z-40 flex items-center justify-around px-2 pb-1 safe-area-pb">
                {tabs.slice(0, 5).map(tab => ( // Show first 5 tabs only on mobile due to space
                    <button
                        key={tab.id}
                        onClick={() => handleTabChange(tab.id)}
                        className={`p-3 rounded-full transition-all duration-300 relative ${activeTab === tab.id
                            ? `${tab.bg} ${tab.color} -translate-y-3 shadow-lg scale-110 ring-4 ring-white`
                            : 'text-gray-400'
                            }`}
                    >
                        {tab.icon}
                ...
                {/* Mobile Menu More/Settings trigger could go here if needed, or just keep simple */}
                <button
                    onClick={() => handleTabChange('settings')}
                    className={`p-3 rounded-full transition-all duration-300 relative ${activeTab === 'settings' ? 'text-gray-800 -translate-y-3 bg-gray-50 shadow-lg ring-4 ring-white' : 'text-gray-400'}`}
                >
